...
MODEL_NAME="DCT_CLF"
DATA_NAME="IMDB"
RESULTS_FOLDER=${DUMP_DIR}/"results/imdb/clf"
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/imdb/clf/greedy.1.txt.seed${seed}" \
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/imdb/clf/ns.p60.1.txt.seed${seed}" \
    --clf-ft-model-path ${DOM_CLF_FT_MODEL} \
    --sim-model-file ${SIM_MODEL} \
    --sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
    --cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/imdb/clf/ns.p90.1.txt.seed${seed}" \
    --clf-ft-model-path ${DOM_CLF_FT_MODEL} \
    --sim-model-file ${SIM_MODEL} \
    --sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
    --cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
