...
MODEL_NAME="DCT"
DATA_NAME="IMDB"
RESULTS_FOLDER=${DUMP_DIR}/"results/imdb/dct"
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/imdb/dct/greedy.1.txt.seed${seed}" \
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/imdb/dct/ns.p60.1.txt.seed${seed}" \
    --clf-ft-model-path ${DOM_CLF_FT_MODEL} \
    --sim-model-file ${SIM_MODEL} \
    --sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
    --cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/imdb/dct/ns.p90.1.txt.seed${seed}" \
    --clf-ft-model-path ${DOM_CLF_FT_MODEL} \
    --sim-model-file ${SIM_MODEL} \
    --sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
    --cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
