...
TRAIN_PROPORTION=1
N_EPOCHS=35
FT_MODEL=${DUMP_DIR}/"ftmodels/political_domclf.model"
...
PRECISION=32
SEED=4567
EXP_NAME="[POLITICAL_CONTRA_5]"
PYTHON_FILE=${PROJECT_DIR}/"dct/pl_arae_contra_train.py"
WANDB_PROJECT_NAME="DCT_CONTRA"
NUM_NEAREST_LABELS=2
WEIGHT_CONTRA_LOSS=1.0

python ${PYTHON_FILE} \
--exp_name ${EXP_NAME} \
--exp_dir "${EXP_DIR}/${EXP_NAME}" \
--src_train_file "${DATA_DIR}/political/democratic.train" \
--trg_train_file "${DATA_DIR}/political/republican.train" \
--src_dev_file "${DATA_DIR}/political/democratic.dev" \
--trg_dev_file "${DATA_DIR}/political/republican.dev" \
--src_test_file "${DATA_DIR}/political/democratic.test" \
--trg_test_file "${DATA_DIR}/political/republican.test" \
--src-train-label-file "${DATA_DIR}/political/democratic.train.labels" \
--trg-train-label-file "${DATA_DIR}/political/republican.train.labels" \
--src-dev-label-file "${DATA_DIR}/political/democratic.dev.labels" \
--trg-dev-label-file "${DATA_DIR}/political/republican.dev.labels" \
--src-test-label-file "${DATA_DIR}/political/democratic.test.labels" \
--trg-test-label-file "${DATA_DIR}/political/republican.test.labels" \
--src-saliency-file "${DATA_DIR}/political/src_dom.attributes.txt" \
--trg-saliency-file "${DATA_DIR}/political/trg_dom.attributes.txt" \
...
--weight-contrastive-loss ${WEIGHT_CONTRA_LOSS}
