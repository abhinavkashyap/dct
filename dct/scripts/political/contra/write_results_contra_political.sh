...
DATA_NAME="POLITICAL"
RESULTS_FOLDER=${DUMP_DIR}/"results/political/contra"
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/political/contra/ns.p60.1.txt.seed${seed}" \
    --clf-ft-model-path ${DOM_CLF_FT_MODEL} \
    --sim-model-file ${SIM_MODEL} \
    --sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
    --cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
...
do
    python ${PYTHON_FILE} \
    --from-file ${FROM_FILE} \
    --to-file ${DUMP_DIR}/"results/political/contra/ns.p90.1.txt.seed${seed}" \
    --clf-ft-model-path ${DOM_CLF_FT_MODEL} \
    --sim-model-file ${SIM_MODEL} \
    --sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
    --cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
