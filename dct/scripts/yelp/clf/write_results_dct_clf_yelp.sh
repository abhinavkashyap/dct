...
#python ${PYTHON_FILE} \
#--from-file ${FROM_FILE} \
#--to-file ${DUMP_DIR}/"results/yelp/clf/greedy.1.txt.seed${seed}" \
...
python ${PYTHON_FILE} \
--from-file ${FROM_FILE} \
--to-file ${DUMP_DIR}/"results/yelp/clf/ns.p60.1.txt.seed${seed}" \
--clf-ft-model-path ${DOM_CLF_FT_MODEL} \
--sim-model-file ${SIM_MODEL} \
--sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
--cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
...
python ${PYTHON_FILE} \
--from-file ${FROM_FILE} \
--to-file ${DUMP_DIR}/"results/yelp/clf/ns.p90.1.txt.seed${seed}" \
--clf-ft-model-path ${DOM_CLF_FT_MODEL} \
--sim-model-file ${SIM_MODEL} \
--sim-sentencepiece-model-file ${SIM_SENTENCEPIECE_MODEL} \
--cola-roberta-checkpoints-dir ${ROBERTA_BASE_CHECKPOINTS_DIR} \
